...
def update_specialty(db: Session, specialty_id: int, specialty_update: schemas.SpecialtyUpdate):
...
def add_specialty_price(db: Session, specialty_id: int, price_create: schemas.SpecialtyPriceCreate):
    db_specialty = get_specialty_by_id(db, specialty_id)
    if not db_specialty:
        return None
...
def get_current_price_for_specialty(db: Session, specialty_id: int) -> Decimal | None:
    price_entry = db.query(models.SpecialtyPrice)\
